...
function quoteYamlDoubleQuoted(value: string): string {
  return `"${value.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`
}

function normalizeSkillDescriptionLine(line: string): string {
  ...
  return `description: ${quoteYamlDoubleQuoted(rawValue)}`
}
...
function generateMarkdown(
  ...
  sections.forEach((section) => {
    const sectionAnchor = `${section.number}-${section.title.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '')}`
...
async function buildSkill(skillConfig: SkillConfig) {
  ...
  // Read section metadata from _sections.md file
  ...
  try {
    ...
    metadata = {
      ...
      abstract: `Guidelines for ${skillConfig.description}.`,
    }
  }
  ...
  // Keep the human-facing SKILL.md trigger description in a YAML-safe format.
  const skillFile = join(skillConfig.skillDir, 'SKILL.md')
  try {
    const skillContent = await readFile(skillFile, 'utf-8')
    const updatedSkillContent = skillContent.replace(
      /^description:\s*.*$/m,
      (line) => normalizeSkillDescriptionLine(line)
    )

    if (updatedSkillContent !== skillContent) {
      await writeFile(skillFile, updatedSkillContent, 'utf-8')
      console.log(`  ✓ Normalized SKILL.md description quoting`)
    }
  } catch {
    // SKILL.md doesn't exist, skip
  }
